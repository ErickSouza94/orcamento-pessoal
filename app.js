...
class BancoDados {
    ...
    recuperarTodosRegistros() {
        ...
        for(let i = 1; i <= id; i++){
            ...
            // Verificação se o localStorage não está vazio
            if(despesa !== null) {
                despesa.id = i
...
function cadastrarDespesa() {
    ...
    function exibirModal(titulo, mensagem, tipo) {
        ...
        // lógica do tipo (sucesso / erro)
        if(tipo === 'sucesso') {
            modalHeader.classList.replace('text-danger', 'text-success')
            modalButton.classList.replace('btn-danger', 'btn-success')
            modalButton.innerHTML = 'Voltar'
        } else if(tipo === 'erro') {
            modalHeader.classList.replace('text-success', 'text-danger')
    ...
    if(despesa.validarDados()){
        bd.gravar(despesa)
        exibirModal('Sucesso!', 'Despesa cadastrada com sucesso!', 'sucesso') 
        limparCampos()
    } else {
       exibirModal('Erro!', 'Existem campos obrigatórios não preenchidos', 'erro')
    }
}
